...
Wingman is a long-lived orchestrator session that runs a crew of agents for you. It is harness-agnostic: `cd` into the repo and run whichever agent CLI you want as your orchestrator - `claude`, `codex`, `grok`, `opencode`, or `pi`, the same five a crew member can already run on via `--agent`. Guard enforcement, onboarding-preferences enforcement, crew spawning, and status commands all work the same on any of the five. Only fleet continuity (the wake loop that resumes the orchestrator's own session automatically after a long wait) is claude-exclusive today, since none of the other four has a wired mechanism for it yet - a gap the orchestrator's own bootstrap names plainly, once, rather than refusing to start.
...
For a `claude`-descriptor crew member, `bin/spawn-crew` gives two further one-time gates (Claude Code's Bypass-Permissions acceptance and each repo's first-time trust dialog) a preflight check before a window opens, refusing the spawn with the exact remedy when it catches either as unmet; once cleared, crew in that repo run unattended. The trust check follows Claude Code's own ancestor-inheritance rule for workspace trust - a plain directory inherits trust from an already-trusted ancestor, but a git repository's own root never inherits trust from anything above it - so a repo trusted only through an ancestor outside its own working tree is correctly refused rather than left to hit a live trust dialog.
...
It also holds `[effort]` (same shape as `[models]`), `[projects.pins]`/`ignore`, `[harness]` (agent, permission mode, Remote Control, runtime backend, tmux session), and `[env]` - a raw `WM_*` passthrough for the ~100 internal tuning knobs the typed settings don't model. Set `backend = "herdr"` to opt in; Herdr requires its CLI and `jq`.
