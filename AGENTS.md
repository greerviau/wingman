...
- **Arm it as a harness-tracked background task** (e.g. Bash `run_in_background`), on its own, never bundled onto the tail of another command.
  ...
  **If I ever relay this arming instruction to a delegate instead of doing it myself** (a lead, via `bin/crew-say` - e.g. after a takeover or resume) - I hand it the mechanism verbatim, not just "arm your own cycle": a Bash call with `run_in_background: true`, on its own; never foreground, never detached; arm nothing if it can't arm it correctly. The incident this rule exists for was exactly a lead told "arm your own watch-fleet cycle" three times and never told *how*, wedged foreground for 5h27m before anyone noticed.
- **On each wake, run `/watch`** - but only if the wake is a background task you armed yourself. It classifies the completed cycle and tells you what to do with each outcome, including the fire reasons that have a specific procedure. Use it for your very first arm of a fresh run too.
  **Exception, while onboarding preferences are unanswered:** `/watch` is a `Skill` call the preferences guard does not allow. Use the raw `Bash` forms instead - `$WINGMAN_BIN/watch-fleet` to arm, `$WINGMAN_BIN/watch-fleet --classify` to process a wake - and switch to `/watch` once preferences resolve.
...
- **A fire reason whose note begins with `REMINDER (`** → a bounded resurface (issue #187), not a fresh delivery: a `review` member with no live dependency watcher goes unmentioned once every `WM_REVIEW_RESURFACE_SECS`. Relay it explicitly as a reminder of already-reported standing work ("still waiting on your review of ... - delivered Xh ago, no watcher currently polling it"), not as a new "ready" announcement, and do **not** re-run the open-questions flow again - it already ran on the original announce.
