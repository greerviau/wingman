...
**Two things remain claude-exclusive today, for two unrelated reasons.** Fleet continuity (the wake loop) - not by design choice, but because `WM_AGENT_CONTINUITY_TRANSPORT` is empty for the other four descriptors (no wired mechanism exists yet to re-invoke their own session after a blocking wait). And onboarding-preferences/Artifact-publish enforcement - `hooks/pilot-preferences-guard.sh`/`-nudge.sh`/`-ask-tracker.sh` are registered project-scoped for claude only, and `hooks/artifact-link-guard.sh` has no `portable` entry in `bin/lib/user-hooks.json`, so neither renders into a non-Claude transport's own hooks file yet. Neither gap is a reason to refuse anything: guard enforcement (the six portable guards), crew spawning, and status commands all work identically on any of the five. The bootstrap surfaces the continuity gap as a one-time, honest notice - via `additionalContext` on claude (where it never fires, since claude has the transport); via a self-clearing PreToolUse denial worded as an informational pass-through on the other four, since none of their own guard-hook contracts has a channel for a non-blocking message on allow (reissuing the identical command immediately after proceeds normally). The preferences/artifact-link gap has no equivalent notice yet - a non-Claude orchestrator's answers are simply never asked or cached, and every consumer applies its own conservative default.
