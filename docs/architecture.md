...
A crew member whose window died shows as `died` and is recoverable either by hand (`bin/crew-takeover <id>` prints the exact resume command) or in bulk via `bin/crew-resume <id>...` / `bin/crew-resume --all-died`, which relaunches it (or every died member) with `claude --resume <session-id>` and verifies the relaunch actually took before flipping it back to `working`. The same death flip, wherever it happens, checks two things it did not used to (issue #251): whether the member's own session transcript still exists on disk under `~/.claude/projects/<repo-slug>/<session-id>.jsonl` - a `died` member with an intact transcript renders as `died (resumable)` in `crew-list`/`board.md`, and `crew-takeover`/`needs-attention` lead with recovery rather than reading as a dead end - and whether its worktree (if it still has one) is dirty, in which case `git stash create` + `git update-ref refs/wip/<id>` anchors the uncommitted state without touching the working tree or moving `HEAD`, so a death mid-edit is never one `rm -rf` away from losing that work.
