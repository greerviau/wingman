...
## Addendum: deregistration (issue #96)

This design covers visibility-on-launch (ask 1) and reconnect-after-drop (ask 2/3) only - it does not cover the reverse direction, deregistering a crew member's Remote-Control-visible session once its work is over.
Issue #96 (a stale/dead crew session still showing as live in Remote Control) investigates that question separately: `docs/plans/2026-07-15-issue-96-remote-control-stale-session-plan.md`.
Its conclusion, confirmed by direct inspection of the installed `claude` CLI binary rather than assumed: `/remote-control` is a stateful toggle with no separate connect/disconnect subcommand and no external API, so a session can only ever be deregistered by running that command inside itself while still alive.
This is fixable for a graceful `bin/crew-standdown` (a live pane still exists to type into) and not fixable at all for an abrupt death (no pane, no process, nothing to act on) - the latter is mitigated with a reporting caveat, not a mechanism, since none exists.
A future reader who wonders whether deregistration was considered as part of this design: it wasn't in scope here, and the answer already exists in the linked plan rather than needing to be re-investigated from scratch.
