...
Check registration first, before suspecting the hook's own matching logic: `bin/lib/sync-user-hooks.py --check --report --settings ~/.claude/settings.json` (from the wingman repo) lists every guard the manifest (`bin/lib/user-hooks.json`) expects that is not currently registered in user-level `settings.json`. Since issue #241, every session-creation path (`bin/wingman`, `bin/spawn-crew`, `bin/crew-resume`) reconciles this automatically and fails closed if it can't, so an unregistered guard here means either a checkout old enough to predate the manifest itself, or a settings file the reconciler could not parse. A registration written mid-run never retrofits an already-running session - Claude Code binds hooks at session start - so the fix is a restart (`systemctl --user restart wingman.service` or equivalent), not just a pull. Check `bin/crew-list` for live crew first if a supervised unit (`systemd/wingman.service.example`, issue #250) is installed: that restart now takes the whole tmux server down, not just the orchestrator's own session, so every live crew member flips to `died` (still resumable by session id) rather than surviving the restart as it would under the older, unsupervised unit.

### `wingman.service` is active but there is no tmux session

This depends on which unit is installed at `~/.config/systemd/user/wingman.service` - the repo ships a reference at `systemd/wingman.service.example` (issue #250), but adopting it on a given machine is a manual step, so an older, unsupervised unit may still be what is actually running.

**Unsupervised unit (`Type=oneshot`, `RemainAfterExit=yes`, `ExecStart=tmux new-session -d ...`):** `~/.wingman/last-launch-failure` is the durable trace for this. `ExecStart` succeeds the instant the session exists - before `bin/wingman` itself has done anything - so `systemctl --user status wingman.service` reads `active` regardless of what happens next. If `bin/wingman` then refuses to start (most commonly: it could not reconcile its user-scope guard hooks, see above), its own `wm_die` message goes to a tmux pane that is destroyed moments later along with the session, reaching nobody. If the file is present, `bin/doctor` also surfaces it near the top of its own output. Read it, fix the named problem, then start wingman again - the file is only cleared by a launch that actually succeeds, so its mere presence means the last attempt refused.

**Supervised unit (`Type=forking` + `PIDFile`, per `systemd/wingman.service.example`):** this unit tracks the tmux **server** process, not `bin/wingman`/`claude` itself, so a server that dies outright no longer produces this symptom - the unit goes `inactive`/`failed` and (`Restart=always`) recovers within seconds on its own. What can still happen: `bin/wingman` refuses to start (same causes and same `last-launch-failure` trace as above) while some *other* session on the same server - the crew session `wingman`, or one started by hand - keeps the server itself alive, so the unit reads `active (running)` with a perfectly healthy `MainPID` and no way to know the orchestrator specifically is gone. Diagnose that case by attaching directly (`tmux attach -t wingman-main`) and checking whether anything is actually running in it.
