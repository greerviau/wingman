...
- **Fleet-continuity transport** - the mechanism, if any, that re-invokes the orchestrator session after a blocking wait. Only `claude` has one (`claude-async-rewake`) today. The other four are not refused for lacking it - the orchestrator's bootstrap surfaces a one-time, honest notice instead (guard enforcement, preferences enforcement, crew spawning, and status commands all still work; only the wake loop itself does not).
