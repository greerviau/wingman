...
- **Source** - which layer a resolved value came from: `env`, `config.local.toml`, `run` (answered this run via `/prefs`), `unanswered` (a preference neither layer answers, so wingman asks or the consumer applies its conservative default), or `default`. `bin/config` reports it per setting.
...
- **Agent descriptor** - the shell-sourceable file at `bin/lib/agents/<name>.sh` describing one agent CLI's launch flags, preflight, and guard/continuity transports. `claude`, `codex`, `grok`, `opencode`, and `pi` are the five this repo ships; a crew member selects one via `--agent`, the orchestrator's own bootstrap (`bin/lib/orchestrator-bootstrap.sh`) via `--agent` too - whichever harness the human actually ran.
- **Guard transport** - the mechanism a descriptor uses to receive wingman's guard hooks (`claude-json`, `codex-json`, `grok-json`, `opencode-plugin`, `pi-extension`). Wingman verifies it by executing the registered command against a known-deny and known-allow fixture, never by inspecting registration alone. The orchestrator's own bootstrap (`bin/lib/orchestrator-bootstrap.sh`, invoked from `hooks/session-init.sh`/`hooks/orchestrator-guard-sync-gate.sh` for claude and from `hooks/lib/guard_dispatch.py` for the other four) installs and verifies its own transport for whichever harness is running; wiring a crew member's own spawn/resume into that same install-and-verify step is a follow-on, not yet done.
- **Fleet-continuity transport** - the mechanism, if any, that re-invokes the orchestrator session after a blocking wait. Only `claude` has one (`claude-async-rewake`) today. The other four are not refused for lacking it - the orchestrator's bootstrap surfaces a one-time, honest notice instead (guard enforcement, crew spawning, and status commands all still work; the wake loop does not). Onboarding-preferences and Artifact-publish enforcement are a separate, also claude-only gap today - `hooks/pilot-preferences-guard.sh` and friends are project-scoped to claude, and `hooks/artifact-link-guard.sh` is not portable - unrelated to the continuity transport specifically.
...
- **Resumable** - a `died` `claude`-descriptor member whose Claude Code session transcript is confirmed present on disk (`is_resumable`, issue #251), independent of whether its worktree survived. Rendered as `died (resumable)` in `crew-list`/`board.md`. Only meaningful for `claude`: `is_resumable()` returns `None`, not `False`, for any other descriptor, since there is no transcript to check for in the first place. A died non-`claude` member is not called resumable, but is not stranded either - `crew-takeover` prints no manual resume command for it (there is none to hand-compose) and points at `crew-resume`'s relaunch mode instead, which recovers it with a fresh session and a progress note rather than a true resume.
- **WIP anchor** - the `refs/wip/<id>` git ref a dirty worktree's uncommitted state (tracked, staged, and untracked files alike) is pointed at, the moment its member flips to `died` (a snapshot commit built through a scratch git index, then `git update-ref`, issue #251). Zero-disruption (neither the working tree, the real index, nor `HEAD` moves) and outside `refs/heads/`, so it is invisible to branch listings. Overwritten, not appended to, on a repeat death - always the latest anchored state, never a history of every death. A failed anchor attempt is recorded as `wip_anchor_error` and surfaced distinctly from "the tree was clean."
