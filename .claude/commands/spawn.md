...
Parse the first token as the crew **type**, the second as the target **repo** (resolved via `bin/discover-projects` if it's a name).
...
If the type isn't recognized, run `bin/spawn-crew --list-types` to show the available crew types and ask me which to use.
