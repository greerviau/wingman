...
Parse the arguments: if the first token names a repo (resolve it via `bin/discover-projects`), spawn the lead there; otherwise, if the work spans repos or the repo is unclear, ground it globally.
Then run one of:
