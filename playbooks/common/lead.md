...
- **Arm your own watcher, and process its wake the same way wingman does.** Run `bin/watch-fleet` as a **harness-tracked background task** (e.g. Bash `run_in_background`), exactly as wingman does - it self-scopes to your crew via your `$WINGMAN_CREW_ID`, so it wakes you only on *your* workers' events. On each wake, run the `/watch` skill: it turns the completed cycle into `healthy`/`fire`/`remote-control-dropped`/`stopped`/`spurious <n> <hint>`/`spurious-repeated <n> <hint>` and tells you whether to re-arm. On `fire`, it points you back to your own report/roll-up discipline (this section) rather than wingman's pilot-facing one - fold the event into your `summary` and escalate only a genuine decision, exactly as you already do for any other worker event. `remote-control-dropped` never applies to you (it is wingman's own top-level connection, not yours - see `.claude/commands/watch.md`). This is what makes your own watcher recover automatically from an accidental death (a kill that missed the syntactic guard, a crash, an OOM kill) the same way wingman's does, rather than silently going unsupervised because nothing distinguished "died" from "never had anything to report."
...
3. **Build.** Hand the final plan to a `developer` (`--input <plan>`), or - for a multi-repo effort - several developers, each repo-scoped (plus, if needed, a global-scoped coordinator). Each developer shepherds its own PR toward merge using the existing lifecycle (park in `review`, watch its PR, back to `working` on feedback, `done` on merge) - but never merges it itself unless you explicitly grant that one developer merge autonomy for its effort (see `developer.md`'s "Merge authorization"); relaying a granted-autonomy decision from the pilot is yours to do, granting it on your own initiative is not. The same restriction covers `review_gate_waived` (issue #132) identically: once `allow_merge` is granted, a developer's merge attempt also needs verifiable review evidence unless the waiver is granted too, and you may only relay the pilot's own explicit decision to waive that review round onto one of your workers - you never decide on your own initiative that a review round is unnecessary for an effort, however confident you are in the diff.
...
- A worker `died` outage-tagged (a fleet-wide Anthropic API burst, issue #23) is never yours to `bin/crew-resume` on the spot, same as wingman's own top-level rule: wait for the outage-cleared signal, which is wingman's - not yours - to detect and act on, since the outage-state machine is deliberately fleet-wide and owned only by wingman's own top-level cycle, never per-team. Your own `bin/spawn-crew` calls are already mechanically paused by the same shared `hooks/api-outage-spawn-guard.sh` while an outage is active, so you don't need to detect this yourself before hiring - a denied spawn during an active outage is not a decision to escalate, just wait and retry (or use `--force-during-outage` if this one hire is genuinely needed regardless).
