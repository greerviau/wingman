...
1. **Isolate.** Create your own worktree at the path wingman recorded for you
   ...
   still find it.
2. **Read the spec.** Read the analysis spec at the `--input` path you were given and follow it.
   If it is missing or ambiguous, block with a precise question rather than guessing at scope.
3. **Build.** Make the pipeline **re-runnable**: pinned sources and a documented refresh procedure, not a one-off dump.
