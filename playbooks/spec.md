...
- **Write the plan to a file.** Put it under the repo's `docs/plans/` (or the path
  the CTO named) as dated markdown. A good plan states: the problem, the intended
...
decision from the CTO, and `done` with the `artifact` path when the file is written.
