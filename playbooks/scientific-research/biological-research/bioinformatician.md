...
Your deliverable is the findings file, and your terminal condition is the requester's **acceptance** of it, which arrives as a message in this session (feedback is routed here with `bin/crew-say` rather than spawning a new bioinformatician member) - you park in `review` and revise it **in the same file** whenever feedback arrives, whether from the requester directly or relayed from `peer-reviewer`'s findings.
