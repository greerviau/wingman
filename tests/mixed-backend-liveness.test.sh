...
inventory="$(wm_backend_herdr_inventory default)"
inv_rc=$?
assert_eq "the inventory read succeeds" "$inv_rc" 0
assert_contains "the inventory interpolates the first tab" "$inventory" \
  '{"backend":"herdr","endpoint":"default:p:1","physical_id":"tab1","label":"wm-alpha","workspace_id":"ws1","tab_id":"tab1","pane_id":"p:1"}'
assert_contains "the inventory interpolates the second tab" "$inventory" \
  '{"backend":"herdr","endpoint":"default:p:2","physical_id":"tab2","label":"wm-beta","workspace_id":"ws1","tab_id":"tab2","pane_id":"p:2"}'
...
endpoints_from_list_live="$(wm_backend_herdr_list_live default | cut -f1 | sort | tr '\n' ' ')"
...
partial="$(WM_FAKE_HERDR_PANE_LIST_FAILS=1 wm_backend_herdr_inventory default)"
assert_false "an unreadable pane list fails the inventory read" \
  'WM_FAKE_HERDR_PANE_LIST_FAILS=1 wm_backend_herdr_inventory default'
...
wm_state crew-add --id herdr-member --type developer --objective H --repo /tmp \
  --window "default:p:1" --session-id s2 --backend herdr >/dev/null
...
wm_state crew-add --id herdr-gone --type developer --objective G --repo /tmp \
  --window "default:p:99" --session-id s3 --backend herdr >/dev/null
wm_state reconcile --inventory "$inventory" --inventory-backends herdr >/dev/null
