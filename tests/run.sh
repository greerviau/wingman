...
#
# stop-continuity.test.sh sits fifth, not first: it's one of the two longest
# files (~333s), so it still needs to start early to keep the pool
# throughput-bound rather than idling on it late (the exact regression a
# prior cut of this list caused - see issue #327/PR #328). But its own case
# 7c' is a known real-watch-fleet-daemon timing race (issue #308, mechanism
# #232) already sensitive to concurrent watch-fleet load, so it should not
# also be the file that fills the very first job-pool wave alongside all
# three watch-fleet-shaped split files at once (issue #334) - ranking it
# right after them means it starts as soon as the first of those four frees
# a slot, not at t=0 alongside two of them.
_wm_priority="group-attention.test.sh watch-fleet-lifecycle.test.sh watch-fleet.test.sh watch-fleet-recovery.test.sh stop-continuity.test.sh crew-resume.test.sh watch-fleet-classify.test.sh forward-motion-check.test.sh stall-recheck.test.sh crew-standdown-remote-control.test.sh busy-pane-refusal.test.sh stall-nudge-confirmation.test.sh playbook-resolution.test.sh wedge-check.test.sh composer-confirm-delivery.test.sh tmux-session-targeting.test.sh stall-check.test.sh outbox-wake-and-retry-log.test.sh session-guard-hook-sync.test.sh"
